...
class ArvoreBn:
    ...
    def sucessor(self,nNoh):
        ...
        if temp:
            return self.minimo(temp.Noh).getId()
        else:
            None
    ...
    def antecessor(self,nNoh):
        ...
        if temp:
            return self.maximo(temp.Noh).getId()
        else:
            None
    def verificar(self,nNoh):
        buscado = self.busca(nNoh)
        if buscado.Noh.getId() == buscado.fDireito.Noh.getId():
            return True
        if self.sucessor(nNoh) == nNoh.getId():
            return True
        else:
            return False
...
lista = [15,4,9,7,18,3,5,16,4,18,17,9,14,5]
for i in lista:
    testearv.add(Noh(i,[i]))
NohBusca = Noh(18,[])
print(testearv.verificar(NohBusca))
